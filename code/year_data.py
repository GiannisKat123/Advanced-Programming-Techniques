...
def energy_per_year(year):
    ...
    for file in os.listdir():
        ...
        if ((os.stat(file_path).st_size == 0) == False and file[4:].startswith(year)) : 
    ...
    dir_path = dir_path[0:-4] + "processed_demands\\"
    print(dir_path)
    os.chdir(dir_path)
    ...
    fig = plt.figure("Energy by year demand")
    ax = fig.add_subplot()

    plt.xticks(x, dates)

    ax.set_title("Average energy demands throughout year, " + year)
    ax.set_xlabel("Date")
    ax.set_ylabel("Average energy per day")
    text = "Maximum energy demanded is: " + str(max) + "\nMinimum energy demanded is: " + str(min) + "\nAverage demand for energy is: " + str(round(avg, 2))
    fig.text(0, 0, text, bbox = dict(boxstyle="square,pad=0.3", fc="pink", ec="gray", lw=1))
